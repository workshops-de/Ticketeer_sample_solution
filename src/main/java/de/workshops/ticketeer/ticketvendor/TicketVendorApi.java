...
public interface TicketVendorApi {
   ...
   * a category is given, then filtered by category. If a city is given, then filtered by city. If
   * both is given then 'and' filtered by category and city.
   *
  ...
  /**
   * GET /event/{id} Get details for a certain event
   *
  ...
  /**
   * POST /event/{id}/order Order tickets for a certain event
   *
   * @param eventId            (required)
   * @param ticketOrderRequest (required)
   * @return Ticket/s successfully ordered (status code 201) or Not enough tickets available or
   * event sold out (status code 400) or Bad request, i.e. event id not found (status code 404) or
  ...
  /**
   * POST /event/{id}/reserve Reserve tickets for a certain event
   *
   * @param eventId                  (required)
