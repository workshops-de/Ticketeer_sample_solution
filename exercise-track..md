...
**Tasks**
1. 1	Clone/open the Ticketeer project.
2. 2	Ensure the app starts with mvn spring-boot:run or via IDE.
3. 3	Add a simple health-like endpoint:
   * ◦	GET /api/ping → returns { "status": "ticketeer-ok" }
4. 4	Use application.properties (or .yml) to set:
   * ◦	server.port=8080
...
**Tasks**
1. 1	Define simple DTOs:
   * ◦	EventDto (id, name, venue, startDateTime, basePrice).
2. 2	Add an in-memory repository:
   * ◦	A List<Event> or Map<Long, Event> stored in a @Component.
3. 3	Implement REST endpoints:
   * ◦	GET /api/events – list all events.
   * ◦	GET /api/events/{id} – details for a single event.
4. 4	Preload 3–5 hard-coded events at startup (e.g. via a DataInitializer bean).
...
**Tasks**
1. 1	Introduce profiles:
   * ◦	application-local.yml
   * ◦	application-test.yml
2. 2	Move seeding config / properties into local profile.
3. 3	Add a configuration properties class:
   * ◦	TicketeerProperties with prefix ticketeer.
   * ◦	Fields like startupBannerEnabled, defaultCurrency, etc.
4. 4	Use these properties in your controller or service (e.g. include defaultCurrency in event responses).
...
**Tasks**
1. 1	Configure **PostgreSQL** datasource via application-local.yml:
   * ◦	spring.datasource.url, username, password.
2. 2	Define JPA entities:
   * ◦	EventEntity (id, name, venue, startDateTime, basePrice, capacity).
   * ◦	SeatInventoryEntity or simply a numeric field on Event for now.
3. 3	Introduce Spring Data repositories:
   * ◦	EventRepository extends JpaRepository<EventEntity, Long>.
4. 4	Replace in-memory repository logic in REST endpoints with database-backed repositories.
...
**Tasks**
1. 1	Add Flyway (or Liquibase) to the project.
2. 2	Create an initial migration:
   * ◦	V1__initial_schema.sql with events table.
3. 3	Remove any ddl-auto=create from config or restrict it to tests only.
4. 4	Add a second migration to:
...
2. 2	Add repositories for both.
3. 3	Implement service methods:
   * ◦	createReservation(eventId, numberOfSeats).
   * ◦	confirmReservation(reservationId) (to be used by order in next exercise).
4. 4	Update events table/entity to track remainingSeats.
...
**Tasks**
1. 1	Implement REST endpoints:
   * ◦	POST /api/events/{id}/reservations with JSON { "seats": 2 }
     * ▪	Returns ReservationDto.
   * ◦	POST /api/reservations/{id}/orders with payment info (simplified).
...
**Story hook:** For a high-profile festival, Ticketeer must sell a portion of tickets from a partner’s legacy system (exposed via an external REST API). Ticketeer will reserve in its own DB but also call out to the partner.
**Tasks**
1. 1	Given an **OpenAPI spec** (vendor-ticket-api.yaml):
   * ◦	Use OpenAPI Generator (or similar) to create a client module or package.
2. 2	Add a Spring bean wrapping the generated client:
...
**Tasks**
1. 1	Create a configuration properties class:
...
3. 3	Inject these properties into VendorTicketClient wrapper.
4. 4	Make external calls conditional on enabled.
...
**Tasks**
1. 1	Introduce Spring Security.
2. 2	Define two categories of endpoints:
   * **◦	Public**:
...
**Tasks**
1. 1	Add an organizerId to Event.
2. 2	Add method-level security:
...
3. 3	Wire this into service methods like publishEvent(...).
...
**Tasks**
1. 1	Add Spring Boot Actuator dependency.
2. 2	Enable basic actuator endpoints in application-local.yml:
   * ◦	management.endpoints.web.exposure.include=health,info,metrics.
3. 3	Configure info with:
...
**Tasks**
1. 1	Implement a custom HealthIndicator:
...
2. 2	Add details to health response:
...
**Tasks**
1. 1	Introduce Micrometer counters or timers:
   * ◦	Counter: ticketeer.reservations.created.
   * ◦	Counter: ticketeer.orders.completed.
2. 2	Increment counters in the respective service methods.
3. 3	Explore /actuator/metrics/ticketeer.reservations.created and /actuator/metrics/ticketeer.orders.completed.
